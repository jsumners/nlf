...
var heading =
  '"name","version","directory","repository","summary",' +
...
function recordToCsv (moduleRecord) {
  const { name, version, directory, repository, licenseSources } = moduleRecord
  const summary = moduleRecord.summary().join(';')
  const { package: pkg, license, readme } = licenseSources
  return `"${name}","${version}","${directory}","${repository}","${summary}","${pkg
    .summary()
    .join(';')}","${license.summary().join(';')}","${readme
    .summary()
    .join(';')}"`
}
