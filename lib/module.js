...
function Module (id, name, version, directory, repository, type) {
  ...
  if (!directory) {
    throw Error('directory must be defined')
  }

  this.id = id
  this.name = name
  this.version = version
  this.repository = (typeof repository === 'string' ? repository : '(none)')
    .replace('git://', 'http://')
    .replace('.git', '')
    .replace(/^git\+/, '')
  ...
  this.licenseSources = {
    package: licenseCollection(),
    license: licenseCollection(),
    readme: licenseCollection()
  }
}
