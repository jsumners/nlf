          ...
          for (name in dependencies) {
            if (dependencies.hasOwnProperty(name)) {
              if (typeof dependencies[name] === 'object') {
                ...
                console.warn(
                  'Warning: dependency "' + name + '" probably not installed.'
                )
                console.warn(
...
function addFiles (filePaths, collection, callback) {
  ...
  for (var i = 0; i < filePaths.length; i += 1) {
    ...
    if (i === lastIndex) end = true
    fileSource(filePath, (err, source) => {
      if (source) {
        collection.add(source)
      }
      if (end === true) return callback(err, source)
      if (err) {
        end = true
        return callback(err)
      }
...
function addPackageJson (moduleData, module) {
  ...
  var license = moduleData.license
  ...
  if (license !== 'none') {
    module.licenseSources.package.add(packageSource(license))
  }
  ...
  if (licenses) {
    ...
    if (Array.isArray(licenses)) {
      for (var index = 0; index < licenses.length; index++) {
        if (licenses[index]) {
          module.licenseSources.package.add(packageSource(licenses[index]))
        }
      }
    } else if (typeof licenses === 'string' || typeof licenses === 'object') {
      // some modules incorrectly have a string or object licenses property
      return module.licenseSources.package.add(packageSource(licenses))
    }
  }
}
...
function createModule (moduleData, callback) {
  var repository =
    (moduleData.repository || {}).url || moduleData.repository || '(none)'
  ...
  findPotentialLicenseFiles(directory, '*li@(c|s)en@(c|s)e*', function (
    err,
    licenseFiles
  ) {
    if (err) {
    ...
    addFiles(licenseFiles, module.licenseSources.license, function (err) {
      ...
      findPotentialLicenseFiles(directory, '*readme*', function (
        err,
        readmeFiles
      ) {
        if (err) {
...
function convertToArray (object) {
  ...
  output.filter(v => v !== null).sort(compareModuleNames)
...
function find (options, callback) {
  ...
  resolveDeps(options.directory, depOptions)
    .then(function (data) {
      // parse the deps data
